...
def kernelized_softmax(query, key, value, kernel_transformation, train_shape, test_shape, projection_matrix=None, edge_index=None, tau=0.25, return_weight=True, return_embedding=False):
    '''
    ...
    if return_weight: # query edge prob for computing edge-level reg loss, this step requires O(E)
        ...
        if return_embedding:
            all_weight = None
            for i in tqdm(range(train_shape, train_shape+test_shape)):
                edge_index = [[i,j] for j in range(train_shape)]
                ...
                edge_attn_dem = attn_normalizer[end]  # [E, B, H]
                edge_attn_dem = edge_attn_dem.permute(1, 0, 2) # [B, E, H]
                weight = edge_attn_num / edge_attn_dem # [B, E, H]

                weight = weight.cpu().numpy()
                weight = np.mean(weight, 2)
                if i == train_shape:
                    all_weight = weight
                else:
                    all_weight = np.concatenate((all_weight, weight))

        else:
            all_weight = None

        return z_output, A_weight, all_weight
...
class GraphTransformerConv(nn.Module):
    ...
    def forward(self, z, adjs, tau, train_shape, test_shape, return_embedding):
        ...
        if self.use_gumbel and self.training:  # only using Gumbel noise for training
            ...
            z_next, weight, all_weight = kernelized_softmax(query, key, value, self.kernel_transformation, train_shape, test_shape, projection_matrix, adjs[0],
                                                tau, self.use_edge_loss, return_embedding=return_embedding)
        ...
        if self.use_edge_loss: # compute edge regularization loss on input adjacency
            ...
            return z_next, link_loss, all_weight
...
class GraphTransformer(nn.Module):
    ...
    def forward(self, x, adjs, tau=1.0, train_shape=0, test_shape=0, return_embedding=False):
        ...
        for i, conv in enumerate(self.convs): 
            if self.use_edge_loss:
                if i == len(self.convs)-1:
                    z, link_loss, weight = conv(z, adjs, tau, train_shape, test_shape, return_embedding)
                else:
                    z, link_loss, weight = conv(z, adjs, tau, train_shape, test_shape, return_embedding=False)
                # z, link_loss, weight = conv(z, adjs, tau, train_shape, test_shape, return_embedding)
